/*
    ...
    Interesting links:
      ...
      (reference)
   https://github.com/daniel5151/AC8E/blob/master/references/Cowgod's%20Chip-8%20Technical%20Reference.pdf
...
size_t chip8_load(char *file) {
    ...
    size_t read = fread(&memory[PROGRAM_START_ADDRESS], sizeof(memory[0]),
                        MAX_PROGRAM_MEMORY, game);
...
void chip8_print_memory(size_t until) {
    for (uint16_t i = PROGRAM_START_ADDRESS;
         i < (PROGRAM_START_ADDRESS + until); i += 2) {
...
static void clear_display() { memset(display, 0, sizeof(uint8_t) * 64 * 32); }
    ...
    switch (type) {
        ...
        case 0x8:
            switch (n) {
                ...
                {
                    cdebug(
                        "SUBN Vx(%X) = Vy(%X) - Vx(%X) (IF Vx > Vy -> Vf = 1 "
                        "ELSE 0)\n",
                        x, y, x);
                ...
                {
                    cdebug(
                        "SHL (*2) Vx(%X) *= 2. IF MSB == 1; Vf = 1 ELSE Vf = "
                        "0\n",
                        x);
        ...
        case 0xF:
            switch (kk) {
                ...
                case 0x0A:  // LD Vx, K
                    // Wait until a key is pressed. We don't advance PC until
                    // any('keyboard') == 1
